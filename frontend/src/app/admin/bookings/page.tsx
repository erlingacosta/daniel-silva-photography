...
export default function BookingsAdmin() {
  ...
  if (loading) {
    return <div className="p-8 text-center text-gray-400"><p>Loading bookings...</p></div>
  }
  ...
  return (
    <div className="p-8">
            ...
            <tbody className="divide-y divide-slate-700">
              {filtered.map(b => (
                <tr key={b.id} className="hover:bg-slate-700 transition-colors">
                  ...
                  <td className="px-4 py-3 whitespace-nowrap">
                    <span className={`px-2 py-1 rounded text-xs font-semibold ${STATUS_COLORS[b.status] || 'bg-gray-700 text-gray-300'}`}>
                      {b.status}
                    </span>
                  </td>
      ...
      {filtered.length === 0 && (
        <div className="mt-8 p-8 text-center text-gray-400"><p>No bookings found</p></div>
      )}
    </div>
  )
}
